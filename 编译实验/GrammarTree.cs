...
namespace 编译实验
{
    ...
    partial class GrammarTree
    {
        ...
        public class GrammarItem
        {
            ...
            public GrammarItem Father;//因为预测分析表的follow集求的不完全，导致插入的词语可能陷入为空的节点中，以该引用进行回溯。如果有完善的follow集的化，则不需回溯。
            ...
            public bool Filled()
            {
                ...
                //    else
                //    {
                //        if (Items[i] != null && ((GrammarItem)Items[i]).Filled())
                //        {
                //            index++;
                //        }
                //    }
            ...
            public void InsertWord(Word wd,ref int counter)
            {
                ...
                else
                {
                    ...
                    if (git.Filled())
                    {
                        ...
                        this.Father.InsertWord(wd,ref counter);//由于Follow集求的不完全 导致需要回溯寻找
            ...
            private int GetAddr(VarTable vt,MLList mlList)
            {
                ...
                if (this.GrammarStr == "1101>1102,1103")
                {
                    ...
                    if (((GrammarItem)this.Items[1]).IsEmpty())
                    {
                        return Left;
                    }
            ...
            private void Make(int Main,int LeftT,VarTable vt,MLList mlList)
            {
                ...
                if(this.GrammarStr== "1103>26,1102,1103"|| this.GrammarStr == "1103>27,1102,1103")
                {
                    ...
                    mlList.Add(((Word)(this.Items[0])).Name, LeftT, now, Main);

                    ((GrammarItem)(this.Items[2])).Make(now, ((GrammarItem)(this.Items[1])).GetAddr(vt,mlList), vt,mlList);
                ...
                if(this.GrammarStr== "1107>24,1101,1107"|| this.GrammarStr == "1107>25,1101,1107")
                {
                    ...
                    if (((GrammarItem)this.Items[2]).IsEmpty())
                    {
                        ...
                        mlList.Add(((Word)(this.Items[0])).Name, LeftT, ((GrammarItem)(this.Items[1])).GetAddr(vt, mlList), Main);
                        return;
                    }
                    var now = vt.AddTemp();
            ...
            private int BoolGetAddr(VarTable vt, MLList mlList,MLList subList)
            {
                if (this.GrammarStr == "1206>38,1207")
                {
                    ...
                    int left = vt.GetVar(((Word)this.Items[0]).Name);
                    int main = vt.AddTemp();

                    ((GrammarItem)this.Items[1]).BoolMake(main,left,vt,mlList, subList);
                    return main;

                }
                if(this.GrammarStr== "1206>43,1200,44")
                {
                    return ((GrammarItem)this.Items[1]).BoolGetAddr(vt, mlList, subList);
                }
                if (this.GrammarStr == "1205>1206")
                {
                    return ((GrammarItem)this.Items[0]).BoolGetAddr(vt, mlList, subList);
                }
                if (this.GrammarStr == "1205>33,1205")
                {
                    int past = ((GrammarItem)this.Items[1]).BoolGetAddr(vt, mlList,subList);
                    int now = vt.AddTemp();
                ...
                {
                    int left = ((GrammarItem)this.Items[0]).BoolGetAddr(vt, mlList, subList);
                    if (((GrammarItem)this.Items[1]).IsEmpty())
                    {
                        return left;
                    }
            ...
            public void CondCommand(VarTable vt, MLList mlList)
            {
                if(this.GrammarStr == "1017>17,1200,18,1011,1022")
                {
                    ...
                    if (this.GetNode(4).IsEmpty())
                    {
                        ...
                        //mlList.List.Insert(insert_jp_false_cmd_ptr, MLanguage.Get("jz", boolAddr, -1, jp2_ptr));
                    }
                    else
                    {
                    ...
                    return;
                    //MessageBox.Show(boolAddr.ToString());
                }
